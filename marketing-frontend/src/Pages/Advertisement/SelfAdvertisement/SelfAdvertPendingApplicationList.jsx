...
function SelfAdvertPendingApplicationList(props) {
    const navigate = useNavigate()


    const { api_getAppliedApplicationList, api_getApprovedApplicationList, api_getRejectedApplicationList, api_getApplicationDetailForPayment } = AdvertisementApiList()
    ...
    const getApprovedApplicationList = () => {
        ...
        axios.post(`${api_getApprovedApplicationList}`, requestBody, ApiHeader())
            .then(function (response) {
                console.log('applied application in self advertisement', response.data.data)
    ...
    const getRejectedApplicationList = () => {
        ...
        axios.post(`${api_getRejectedApplicationList}`, requestBody, ApiHeader())
            .then(function (response) {
    ...
    const handlePayment = (e) => {
        ...
        axios.post(`${api_getApplicationDetailForPayment}`, requestBody, ApiHeader())
            .then(function (response) {
    ...
    return (
        <>
            {/***** section 3 ******/}
            <div className=''>
                <AdvertPaymentModal openPaymentModal={openPaymentModal} applicationDetails={applicationIdDetail} showLoader={props.showLoader} />

            </div>

            <div className=''>
                <div className='flex flex-col md:flex lg:flex  bg-white p-2 mb-4 shadow-md text-sm md:text-lg lg:text-lg font-semibold text-gray-500  '>
                    {tabIndex == 1 && <h1 className='flex-1'>Self Advertisement <span className='text-indigo-500 underline'>Pending Application</span> </h1>}
                    {tabIndex == 2 && <h1 className='flex-1'>Self Advertisement <span className='text-indigo-500 underline'>Approved Application</span></h1>}
                    {tabIndex == 3 && <h1 className='flex-1'>Self Advertisement <span className='text-indigo-500 underline'>Rejected Application</span></h1>}
                    <div className=' flex-1  md:-mt-7 lg:-mt-7'>
                        <button type='button' className='text-xs md:text-sm lg:text-sm bg-indigo-300 px-1 py-1 mr-2 text-indigo-600 border border-indigo-500  rounded leading-5 float-right focus:bg-indigo-500  focus:shadow-lg focus:text-white dark:bg-white' onClick={() => settabIndex(3)}>Rejected Application</button>
                        <button type='button' className='text-xs md:text-sm lg:text-sm bg-indigo-300 px-1 py-1 mr-2 text-indigo-600 border border-indigo-500  rounded leading-5 float-right focus:bg-indigo-500  focus:shadow-lg focus:text-white dark:bg-white' onClick={() => settabIndex(2)}>Approved Application</button>
                        <button type='button' className='text-xs md:text-sm lg:text-sm bg-indigo-300 px-1 py-1 mr-2 text-indigo-600 border border-indigo-500  rounded leading-5 float-right focus:bg-indigo-500  focus:shadow-lg focus:text-white dark:bg-white' onClick={() => settabIndex(1)}>Pending Application</button>
                                ...
                                {appliedApplication?.data?.map((items) => (
                                    <div className='col-span-3 p-3 h-auto mb-4 shadow-lg rounded leading-5 bg-white transform transition duration-300 ease-in-out hover:-translate-y-2 hover:shadow-xl  '>
                                        ...
                                        <div className='grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 bg-yellow-500  w-36 -ml-3 rounded-r-lg shadow-md top-0 absolute '>
                                            <h1 className='text-xs  text-center text-white '>Self Advertisement</h1>
                                        </div>
                                        ...
                                        <div className='grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1  p-2'>
                                            <div className='flex-1 justify-end'>
                                                ...
                                                {items.payment_status == 1 ?
                                                    ...
                                                    :
                                                    <button type="button" id={items?.id} value={items?.payment_amount} class="float-right text-xs  px-4 inline-block text-center  rounded leading-5 text-gray-100 bg-indigo-500 border border-indigo-500 hover:text-white hover:bg-indigo-600 hover:ring-0 hover:border-indigo-600 focus:bg-indigo-600 focus:border-indigo-600 focus:outline-none focus:ring-0 mr-2" onClick={handlePayment}>Pay</button>
                                                }
                                            </div>
                                        </div>
                                    </div>
            ...
            >
                <div class=" rounded-lg shadow-xl border-2 border-gray-50 mx-auto px-0 " style={{ 'width': '80vw', 'height': '100%' }}>
                    <ViewAppliedApplication data={applicationNo} applicationType={applicationType} showLoader={props.showLoader} closeModal={closeModal} />
                </div>
            </Modal>
...
export default SelfAdvertPendingApplicationList