...
import PrivateLandPaymentModal from './PrivateLandPaymentModal';
import PrivateLandApplicationFullDetail from './PrivateLandApplicationFullDetail';
...
function PrivateLandPendingApplications(props) {
    ...
    const { api_getPrivateLandAppliedApplicationList, api_getPrivateLandApprovedApplicationList, api_getPrivateLandRejectedApplicationList, api_getPrivateLandApplicationDetailForPayment } = AdvertisementApiList()
    ...
    const getApprovedApplicationList = () => {
        ...
        axios.post(`${api_getPrivateLandApprovedApplicationList}`, requestBody, ApiHeader())
            .then(function (response) {
                console.log('Private land approved list', response.data.data)
    ...
    console.log("approved application list private land...", approvedApplication)
    ...
    const getRejectedApplicationList = () => {
        ...
        axios.post(`${api_getPrivateLandRejectedApplicationList}`, requestBody, ApiHeader())
            .then(function (response) {
    ...
    const handlePayment = (e) => {
        ...
        axios.post(`${api_getPrivateLandApplicationDetailForPayment}`, requestBody, ApiHeader())
            .then(function (response) {
    ...
    return (
        <>
            ...
            <PrivateLandPaymentModal openPaymentModal={openPaymentModal} applicationDetails={applicationIdDetail} showLoader={props.showLoader} />

            <div className=''>
                <div className='flex flex-col md:flex lg:flex bg-white p-2 mb-4 shadow-md  text-lg  font-semibold text-gray-500  '>
                    {tabIndex == 1 && <h1 className='flex-1'>Private Land <span className='text-indigo-500 underline'>Pending Application</span> </h1>}
                    {tabIndex == 2 && <h1 className='flex-1'>Private Land <span className='text-indigo-500 underline'>Approved Application</span></h1>}
                    {tabIndex == 3 && <h1 className='flex-1'>Private Land <span className='text-indigo-500 underline'>Rejected Application</span></h1>}
                                ...
                                {appliedApplication?.data?.map((items) => (
                                    <div className='col-span-3 p-3 h-auto mb-4 shadow-lg rounded leading-5 bg-white transform transition duration-300 ease-in-out hover:-translate-y-2 hover:shadow-xl  '>
                                        <div className='grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 bg-yellow-500  w-36 -ml-3 rounded-r-lg shadow-md top-0 absolute '>
                                            <h1 className='text-xs  text-center text-white '>Private Land</h1>
                                        </div>
            ...
            >
                <div class=" rounded-lg shadow-xl border-2 border-gray-50 mx-auto px-0" style={{ 'width': '80vw', 'height': '100%' }}>
                    <PrivateLandApplicationFullDetail data={applicationNo} applicationType={applicationType} showLoader={props.showLoader} closeModal={closeModal} />
                </div>
            </Modal>
