import React from 'react'

function ReviewMovableApplication(props) {
    let labelStyle = "mt-6 -ml-7 text-xs text-gray-600"
    let inputStyle = "mt-6 -ml-7 text-md text-gray-800 font-bold"

    const submitForm = () => {
        props.submitFun()
    }
    console.log("data in review form...", props?.allFormData)
    console.log("select data in review form ...", props?.reviewIdNameData)

    return (
        <>

            <div>
                <div className='grid grid-cols-1 md:grid-cols-12 lg:grid-cols-12 gap-4 container mx-auto  '>
                    <div className='col-span-8 border border-dashed border-violet-800 -mt-[35rem]'>
                        <h1 className='text-center p-3 mb-2 bg-white text-gray-600 font-sans font-semibold'>APPLICATION DETAILS</h1>
                        <div className='grid grid-cols-1 md:grid-cols-4 lg:grid-cols-4 p-2 ml-12'>
                            <div className=''>
                                <p className={`${labelStyle}`}>Ulb</p>
                                <span className={`${inputStyle}`}>{props?.reviewIdNameData?.movableVehicle?.ulb}</span>
                            </div>
                            <div className=''>
                                <p className={`${labelStyle}`}>License From</p>
                                <span className={`${inputStyle}`}>{props?.allFormData?.movableVehicle?.licenseFrom}</span>
                            </div>
                            <div className=''>
                                <p className={`${labelStyle}`}>License To  </p>
                                <span className={`${inputStyle}`}>{props?.allFormData?.movableVehicle?.licenseTo}</span>
                            </div>
                            <div className=''>
                                <p className={`${labelStyle}`}>Applicant</p>
                                <span className={`${inputStyle}`}>{props?.allFormData?.movableVehicle?.applicantName}</span>
                            </div>
                        </div>
                        <div className='grid grid-cols-1 md:grid-cols-4 lg:grid-cols-4 p-2 ml-12'>
                            <div className=''>
                                <p className={`${labelStyle}`}>Residence Address</p>
                                <span className={`${inputStyle}`}>{props?.allFormData?.movableVehicle?.residenceAddress}</span>
                            </div>
                            <div className=''>
                                <p className={`${labelStyle}`}>Residence Ward No </p>
                                <span className={`${inputStyle}`}>{props?.reviewIdNameData?.movableVehicle?.residenceWardNo}</span>
                            </div>
                            <div className=''>
                                <p className={`${labelStyle}`}>Permanent Address</p>
                                <span className={`${inputStyle}`}>{props?.allFormData?.movableVehicle?.permanentAddress}</span>
                            </div>
                            <div className=''>
                                <p className={`${labelStyle}`}>Permanent Ward No</p>
                                <span className={`${inputStyle}`}>{props?.reviewIdNameData?.movableVehicle?.permanentWardNo}</span>
                            </div>
                        </div>
                        <div className='grid grid-cols-1 md:grid-cols-4 lg:grid-cols-4 p-2 ml-12'>

                            <div className=''>
                                <p className={`${labelStyle}`}>Father Name</p>
                                <span className={`${inputStyle}`}>{props?.allFormData?.movableVehicle?.fatherName}</span>
                            </div>

                            <div className=''>
                                <p className={`${labelStyle}`}>Aadhar No</p>
                                <span className={`${inputStyle}`}>{props?.allFormData?.movableVehicle?.aadharNo}</span>
                            </div>
                            <div className=''>
                                <p className={`${labelStyle}`}>Mobile No. </p>
                                <span className={`${inputStyle}`}>{props?.allFormData?.movableVehicle?.mobileNo}</span>
                            </div>

                            <div className=''>
                                <p className={`${labelStyle}`}>E-mail </p>
                                <span className={`${inputStyle}`}>{props?.allFormData?.movableVehicle?.email}</span>
                            </div>
                        </div>
                        <div className='grid grid-cols-1 md:grid-cols-4 lg:grid-cols-4 p-2 ml-12'>

                            <div className=''>
                                <p className={`${labelStyle}`}>Entity Name</p>
                                <span className={`${inputStyle}`}>{props?.reviewIdNameData?.movableVehicle?.entityName}</span>
                            </div>
                            <div className=''>
                                <p className={`${labelStyle}`}>GST No</p>
                                <span className={`${inputStyle}`}>{props?.reviewIdNameData?.movableVehicle?.gstNo}</span>
                            </div>
                            <div className=''>
                                <p className={`${labelStyle}`}>Vehicle No</p>
                                <span className={`${inputStyle}`}>{props?.allFormData?.movableVehicle?.vehicleNo}</span>
                            </div><div className=''>
                                <p className={`${labelStyle}`}>Vehicle Name</p>
                                <span className={`${inputStyle}`}>{props?.allFormData?.movableVehicle?.vehicleName}</span>
                            </div>
                        </div>
                        <div className='grid grid-cols-1 md:grid-cols-4 lg:grid-cols-4 p-2 ml-12'>

                            <div className=''>
                                <p className={`${labelStyle}`}>Vehicle Type</p>
                                <span className={`${inputStyle}`}>{props?.allFormData?.movableVehicle?.vehicleType}</span>
                            </div>
                            <div className=''>
                                <p className={`${labelStyle}`}>Brand in Vehicle</p>
                                <span className={`${inputStyle}`}>{props?.allFormData?.movableVehicle?.brandDisplayedInVehicle}</span>
                            </div>
                            <div className=''>
                                <p className={`${labelStyle}`}>Front Area(Sq ft)</p>
                                <span className={`${inputStyle}`}>{props?.allFormData?.movableVehicle?.frontArea}</span>
                            </div><div className=''>
                                <p className={`${labelStyle}`}>Rear Area(Sq ft) </p>
                                <span className={`${inputStyle}`}>{props?.reviewIdNameData?.movableVehicle?.rearArea}</span>
                            </div>
                        </div>
                        <div className='grid grid-cols-1 md:grid-cols-4 lg:grid-cols-4 p-2 ml-12'>
                            <div className=''>
                                <p className={`${labelStyle}`}>Side 1 Area(Sq ft)</p>
                                <span className={`${inputStyle}`}>{props?.allFormData?.movableVehicle?.sideOneArea}</span>
                            </div>
                            <div className=''>
                                <p className={`${labelStyle}`}>Top Area(Sq ft)</p>
                                <span className={`${inputStyle}`}>{props?.allFormData?.movableVehicle?.topArea}</span>
                            </div>
                            <div className=''>
                                <p className={`${labelStyle}`}>Display Type</p>
                                <span className={`${inputStyle}`}>{props?.allFormData?.movableVehicle?.displayType}</span>
                            </div>
                            <div className=''>
                                <p className={`${labelStyle}`}>Trade License No</p>
                                <span className={`${inputStyle}`}>{props?.allFormData?.movableVehicle?.tradeLicenseNo}</span>
                            </div>
                        </div>
                    </div>
                    {/* document */}
                    {/* <div className='col-span-4 border border-dashed border-violet-800 -mt-[35rem]'>
                    <h1 className='text-center p-3 mb-2 bg-white text-gray-600 font-sans font-semibold '>DOCUMENTS UPLOADED</h1>

                    <div className='grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 p-2 ml-12'>
                        {props?.allFormData?.movableVehicleDoc?.map((data, index) => (
                            <div className=''>

                                <p className={`${labelStyle}`}>Aadhar Document</p>
                                <div className='flex -mt-2'>
                                    <span className={`${inputStyle} flex-1`}>new-product.png</span>
                                    <span className={`mt-4  flex-1`}><img src="https://cdn-icons-png.flaticon.com/512/5719/5719894.png" alt="Preview Image" className={` w-8`} /></span>
                                </div>
                            </div>
                        ))}
                        <div className=''>
                            <p className={`${labelStyle}`}>Trade License</p>
                            <div className='flex -mt-2'>
                                <span className={`${inputStyle} flex-1`}>new-product.png</span>
                                <span className={`mt-4 flex-1`}><img src="https://cdn-icons-png.flaticon.com/512/5719/5719894.png" alt="Preview Image" className={` w-8`} /></span>
                            </div>
                        </div>
                        <div className=''>
                            <p className={`${labelStyle}`}>GPS Mapped Camera</p>
                            <div className='flex -mt-2'>
                                <span className={`${inputStyle} flex-1`}>new-product.png</span>
                                <span className={`mt-4 flex-1`}><img src="https://cdn-icons-png.flaticon.com/512/5719/5719894.png" alt="Preview Image" className={` w-8`} /></span>
                            </div>
                        </div>
                        <div className=''>
                            <p className={`${labelStyle}`}>Holding No. Photograph</p>
                            <div className='flex -mt-2'>
                                <span className={`${inputStyle} flex-1`}>new-product.png</span>
                                <span className={`mt-4 flex-1`}><img src="https://cdn-icons-png.flaticon.com/512/5719/5719894.png" alt="Preview Image" className={` w-8`} /></span>
                            </div>
                        </div>
                        <div className=''>
                            <p className={`${labelStyle}`}>GST Document</p>
                            <div className='flex -mt-2'>
                                <span className={`${inputStyle} flex-1`}>new-product.png</span>
                                <span className={`mt-4 flex-1`}><img src="https://cdn-icons-png.flaticon.com/512/5719/5719894.png" alt="Preview Image" className={` w-8`} /></span>
                            </div>
                        </div>
                        <div className=''>
                            <p className={`${labelStyle}`}>Brand Display Permission</p>
                            <div className='flex -mt-2'>
                                <span className={`${inputStyle} flex-1`}>new-product.png</span>
                                <span className={`mt-4 flex-1 `}><img src="https://cdn-icons-png.flaticon.com/512/5719/5719894.png" alt="Preview Image" className={` w-8`} /></span>
                            </div>
                        </div>
                    </div>
                </div> */}
                </div>
                <div className="grid grid-cols-12 w-full p-3">
                    <div className='md:pl-0 col-span-6'>
                        <button type="button" class="text-xs py-2 px-4 inline-block text-center mb-3 rounded leading-5 text-gray-100 bg-indigo-500 border border-indigo-500 hover:text-white hover:bg-indigo-600 hover:ring-0 hover:border-indigo-600 focus:bg-indigo-600 focus:border-indigo-600 focus:outline-none focus:ring-0" onClick={() => props.backFun(3)}>back</button>

                    </div>
                    <div className='col-span-6'>
                        <button type="submit" class="float-right text-xs py-2 px-4 inline-block text-center mb-3 rounded leading-5 text-gray-100 bg-green-500 border border-green-500 hover:text-white hover:bg-green-600 hover:ring-0 hover:border-green-600 focus:bg-green-600 focus:border-green-600 focus:outline-none focus:ring-0" onClick={submitForm}>Submit</button>

                    </div>
                </div>
            </div>
        </>
    )
}

export default ReviewMovableApplication